...
class SemanticRenderer(Renderer):
    ...
    def __init__(self, cfg: CfgNode, faces: np.array, lbs: np.array, viewport_size=(768, 768)):
        """
        ...
        vertex_colors = cv2.applyColorMap((10*self.vertex_labels).astype(np.uint8) , cv2.COLORMAP_VIRIDIS)
        ...
        self.semantic_background_rgb = np.squeeze(semantic_background_rgb.astype(np.float32), axis=1) / 255
        self.renderer = pyrender.OffscreenRenderer(viewport_width=viewport_size[0],
    ...
    def __call__(self,
        ...
        color, rend_depth = renderer.render(scene, flags=pyrender.RenderFlags.FLAT | pyrender.RenderFlags.RGBA)
    ...
    def render_all(self,
        ...
        valid_mask = (color[:, :, -1])[:, :, np.newaxis]
        semantic_map = color[:, :, :3] * valid_mask + (1 - valid_mask) * self.semantic_background_rgb
        semantic_map = semantic_map.astype(np.float32)
        
        if not side_view and not top_view:
            output_img = (color[:, :, :3] * valid_mask + (1 - valid_mask) * image[:,:,::-1])
        else:
            output_img = color[:, :, :3]

        output_img = output_img.astype(np.float32)
        return {"Image": output_img, "Mask":valid_mask.astype(bool), "SemanticMap": semantic_map, "Depth": rend_depth}
    ...
    def render_all_multiple(
        ...
        trimesh_list = [self.vertices_to_trimesh(vvv, ttt.copy(), mesh_base_color, rot_axis, rot_angle) for vvv,ttt in zip(vertices, cam_t)]
        ...
        return {"Image": color, "Mask":valid_mask.astype(bool), "SemanticMap": semantic_map, "Depth": rend_depth}
