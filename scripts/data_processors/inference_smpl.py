...
if __name__ == "__main__":
    ...
    for img_path in tqdm(reference_img_paths, desc="Processing Reference Images:"):
        ...
        for batch in dataloader:
            ...
            smpl_output = model.smpl(
                **{k: v.float() for k, v in pred_smpl_parameter.items()}, pose2rot=False
            )
            ...
            cv2.imwrite(
                os.path.join(
                    args.reference_imgs_folder, "visualized_imgs", f"{img_fn}.png"
                ),
                255 * cam_view[:, :, ::-1],
            )
            cv2.imwrite(
            ...
            cv2.imwrite(
                os.path.join(
                    args.reference_imgs_folder, "semantic_map", f"{img_fn}.png"
                ),
            ...
            np.save(
                str(
                    os.path.join(
                        args.reference_imgs_folder, "smpl_results", f"{img_fn}.npy"
                    )
                ),
                results_dict_for_rendering,
            )
    for video_path in tqdm(driving_videos_paths, desc="Processing Driving Videos:"):
        ...
        np.savez(
            str(os.path.join(video_path, f"smpls_group.npz")), smpl=smpls, camera=cams
        )
